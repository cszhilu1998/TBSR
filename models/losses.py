...
def normalize_batch(batch_in):
	batch = batch_in.clamp(1e-8, 1) ** (1.0 / 2.2)
	batch_out = torch.flip(batch, dims=[1])
	mean = batch_out.new_tensor([0.485, 0.456, 0.406]).view(-1, 1, 1)
	std = batch_out.new_tensor([0.229, 0.224, 0.225]).view(-1, 1, 1)
	return (batch_out - mean) / std
