...
from data.data_processing.camera_pipeline import demosaic
...
class BaseModel(ABC):
	...
	def get_current_visuals(self):
		...
		if self.isTrain:
			for name in self.visual_names:
				if 'data_lr' in name:
					visual_ret[name] = demosaic(getattr(self, name)[0].detach()) * 255
					visual_ret[name] = torch.clamp(visual_ret[name], 0, 255).round().unsqueeze(dim=0)
				else:	
					visual_ret[name] = torch.clamp(
							getattr(self, name)[0].detach()*255, 0, 255).round().unsqueeze(dim=0)
		else:
			for name in self.visual_names:
	...
	def get_backwarp(self, tenFirst, tenSecond, net, flow=None):
		if flow is None:
			flow = self.get_flow(self.gamma(tenFirst), self.gamma(tenSecond), net)
	...
	def gamma(self, batch_in):
		batch = batch_in.clamp(1e-8, 1) ** (1.0 / 2.2)
		batch_out = torch.flip(batch, dims=[1])
		return batch_out
